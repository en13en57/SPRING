...
public class MainApp3 {
	public static void main(String[] args) {
		// 이 순간 스프링 프레임워크가 설정XML파일을 읽어서 등록된 모든 객체를 적재한다.
		// 스프링을 경량 컨테이너다 -- 애플리케이션에서 사용되는 모든 객체를 저장하고 생성부터 소멸까지 관리를 해준다.
		AbstractApplicationContext context = new AnnotationConfigApplicationContext(HelloWorldConfig.class); // context에 다 넣어줌
		...
		// 객체를 2개의 변수를 선언해서 사용했지만 객체는 1개다. 해시코드가 같다. (알아서 싱글톤 됨)
