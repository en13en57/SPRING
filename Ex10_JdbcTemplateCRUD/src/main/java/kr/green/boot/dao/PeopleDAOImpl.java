...
public class PeopleDAOImpl implements PeopleDAO {
	...
	public boolean insert(PeopleVO peopleVO) {
		return jdbcTemplate.update("insert into people value (?,?,?,?)",
									peopleVO.getId(), peopleVO.getFirst_name(), peopleVO.getLast_name(), peopleVO.getAge())>0;
	}

	@Override
	public boolean update(PeopleVO peopleVO) {
		return jdbcTemplate.update("update people set first_name=?, last_name=?, age=? where id=?",
									peopleVO.getFirst_name(), peopleVO.getLast_name(), peopleVO.getAge(), peopleVO.getId())>0;
	}
